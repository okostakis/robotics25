...
async def main():
    ...
    disStep=7
    while True:
        r.followLine(sensorIndex=1,speed=30,leftRight=-1,kp=1,target=70)
        dist=r.getDistance()
        if dist-lastDistance>=disStep:
            if len(ls1)<3:
                ls1.append(round(r.color1.getLight()))
            else:
                ls1[lsI % 3]=round(r.color1.getLight())
            ls1.sort()
            ls2=ls1
            lsI+=1

            lastDistance=dist
        if r.getDistance()>=100 :
            if ls2[1]<5:
                r.stop()
